...
public class EnemyBossAI : EnemyAI
{
    ...
    public override void CheckDistance()
    {
        ...
        if (pos <= maxDistance)
        {
            ...
            if (pos < minDistance)
            {
                //Stop Moving
                move = false;
                if(pos < minDistance - 2.2f && stats.currentHealth > 50f)
                {
                    //move backwards
    ...
    void Update()
    {
        ...
        FaceTarget();
        if (player != null && stats.currentHealth > 50f)
        {
    ...
    void Phases(bool IsAttacking)
    {
        if(stats != null)
        {
            if (stats.currentHealth > 50f) //Initial phase
            {
                //Do a shooting attack
                phase1Attack(IsAttacking);
            }
            else if (stats.currentHealth <= 50f)//Second phase
            {
                //Do a melee attack
                setParameters();
